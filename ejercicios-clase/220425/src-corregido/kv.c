...
static int kv_evict1()
{
    ...
    list_unlink(&kv->lru);
    list_unlink(&kv->list);
    pthread_spin_unlock(&spinlockLRU);  // Nos aseguramos de liberar la LRU cuando estamos seguros de haber eliminado de la tabla

    __kv_release(kv);
    pthread_mutex_unlock(&mutexes[bin % mutexesSZ]); // Unlock acá para evitar conflicto de releases por referencias en __kv_release
    return r;
}
...
int kv_add(int klen, char *k, int vlen, char *v)
{
    ...
    if (old) {
        pthread_spin_lock(&spinlockLRU);            // Evitamos que la LRU pueda tener elementos que son eliminados por un momento
    ...
    pthread_spin_lock(&spinlockLRU);                // Evitamos que la LRU pueda no tener elementos que se están insertando por un momento
    list_add_head(&Table[idx].vals, &new->list);
    list_add_head(&LRU, &new->lru);
    pthread_spin_unlock(&spinlockLRU);

    pthread_mutex_unlock(&mutexes[idx % mutexesSZ]); // Desbloqueamos después, sino otro hilo lo puede cambiar cuando aun 
...
struct kv * kv_take(int klen, const char *k)
{
    ...
    if (ret) {
        ...
        pthread_spin_lock(&spinlockLRU);        // Evitamos que la LRU tenga por algún momento elementos que se eliminan de la lista
    ...
    for (i = 0; i < mutexesSZ; i++) {
        ...
        for (j = i; j < TableSZ; j += mutexesSZ)
            list_for_each_safe(p, swap, &Table[j].vals) {
                ...
                pthread_spin_lock(&spinlockLRU);        // Evitamos que la LRU tenga elementos eliminándose / ya eliminados
                list_unlink(&kv->list);
                list_unlink(&kv->lru);
                pthread_spin_unlock(&spinlockLRU);
